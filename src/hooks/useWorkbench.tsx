...
export const useWorkbench = () => {
  ...
  const onRun = useCallback(
    async (id?: string) => {
      ...
      if (!task?.openAIOptions?.baseURL || !task?.openAIOptions?.apiKey) {
        return toast('Check the settings', {
          ...
          action: {
            label: 'Set now',
            onClick: () => navigate(`/${params.taskId}/settings`),
          },
        })
      }
      if (qas.filter((qa) => !!qa.question.trim()).length === 0) {
        return toast('No question', {
      ...
      const _selectIds = id ? [id] : selectIds
      for (let i = 0, len = updatedQas.length; i < len; i++) {
        if (!updatedQas[i].question.trim()) continue
        if (_selectIds.length > 0 && !_selectIds.includes(updatedQas[i].id))
          continue
    ...
    [task, qas, params, navigate, selectIds]
  )
